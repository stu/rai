...
INIFILE* INI_EmptyINF(void)
{
	...
	f->lstGroups = calloc(1, sizeof(DList));
...
void INI_UpdateItem(INIFILE *f, char *group, char *key, char *val)
{
	...
	while (egroup != NULL)
	{
		...
		if (stricmp(gname, g->name) == 0)
		{
			...
			i = calloc(1, sizeof(struct udtItem));
			i->name = strdup(key);
			i->value = strdup(val);
	...
	g = calloc(1, sizeof(struct udtGroup));
	g->name = strdup(gname);
	g->lstItems = calloc(1, sizeof(DList));
			...
			*line = 0x0;
			...
			if (*line != 0x0)
			{
				...
				if (*line != 0)
				{
					...
					{
						current_group = calloc(1, sizeof(struct udtGroup));
						current_group->name = strdup(sym);
						current_group->lstItems = calloc(1, sizeof(DList));
